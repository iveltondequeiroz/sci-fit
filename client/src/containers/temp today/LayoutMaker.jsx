	...
	generateProgram(){
		...
		if(this.state.templateName!='none') {		
       	...
       	this.setState({
			...
			let time = moment().format('DDMMMYY')
...
*/
    ...
    render(){
    	console.log("------------>RENDER<-------------")
    	/*console.log(this.state.studentName)
    	console.log("this.state.zeroProgram")
		console.log(this.state.zeroProgram)
		console.log("this.state.howmanydays")
		console.log(this.state.howmanydays)
*/
  
    	/*if(this.state.info == "loading ...") {
    		return (
    			 <CircularProgress />  	console.log("this.state.zeroProgram")
		console.log(this.state.zeroProgram)
    	...
    	*/
    	...
    	if(this.state.zeroProgram || this.state.studentName=="none"){
			...
			<div className="container-fluid">	
				<ToolbarLayout title="Program Layout Maker"/>	
				<Grid fluid={true}>
			...
			const mappedExercises = []
		 	...
		 	let lastWeekDays = []
		 	if(this.state.selWeek!=0){
		 		console.log("%%%% lastWeeekDays")
		 		let ndays = this.state.program.weeks[this.state.selWeek-1].days.length
		 		for(let nd=0;nd<ndays;nd++){
		 			let lastweekday = this.state.program.weeks[this.state.selWeek-1].days[ndays-1]
		 			lastWeekDays.push(lastweekday)		
		 		}	
		 	}	
	    	...
	    	for(let d=0;d<programDays;d++) {
				...
				let pfsidrop='', pfsival='', pnote=''
				let fullSetsDay = []	

					
				if(this.state.selWeek!=0 && d==0){
					let nodaysPreviousDay = this.state.program.weeks[this.state.selWeek-1].days.length
					let prevSetsDay = this.state.program.weeks[this.state.selWeek-1].days[nodaysPreviousDay-1].sets
					for(let s=0;s<setsDay.length;s++){

						pw1='',pw2='',pw3='',pw4='',pw5='',pw6=''
						pr1='',pr2='',pr3='',pr4='',pr5='',pr6=''
						pfsidrop='', pfsival='', pnote=''
	
						console.log(">>>>>setsDay>>", setsDay[s])
						if(setsDay[s].amrap==true){
							console.log("searching ...setsday["+s+"]=", setsDay[s].id )
							result = this.prevDayVal(setsDay[s].id, 0, this.state.selWeek-1)
						} else {
							result='none'
						}
					...
					console.log("entrou no ELSE, day=",d)
					for(let s=0;s<setsDay.length; s++){
						...
						if(this.state.selWeek==0 && d==0) {
							// display none
						} else if(this.state.selWeek==0) {
							if(setsDay[s].amrap==true) {
								console.log("searching ...setsday["+s+"]=", setsDay[s].id )
								result = this.prevDayVal(setsDay[s].id, d, this.state.selWeek)
							} else {
								result='none'
							}	
							if(result!='none') {
							...
							console.log("NO ELSE NO ELSE NO ELSE NO ELSE NO ELSE NO ELSE NO ELSE NO ELSE NO ELSE ")
							console.log("pw1",pw1)
							if(setsDay[s].amrap==true) {
								console.log("searching ...setsday["+s+"]=", setsDay[s].id )
								result = this.prevDayVal(setsDay[s].id, d, this.state.selWeek)
						...
						console.log(pw1,pw2,pw3,pw4,pw5,pw6)
						console.log(pr1,pr2,pr3,pr4,pr5,pr6)

						let fsitext = this.fsiText(pfsidrop, pfsival)
						...
						<TableRow key={i+1}>
							...
							<TableRowColumn>
								<select value={fullSetsDay[i].fsidrop}
					        		...
					        		<option value="1" id="1">Prescribed RIR</option>
									<option value="2" id="2">Choose RIR</option>
									<option value="3" id="3">Go to Failure On Final Set</option>
								</select>
		...
		<div className="container-fluid">	
        	...
        	<Grid fluid={true}>
				<Row className="show-grid">
        			<Col xs={9} md={9} lg={9}>
		        		<Card>
		        			<CardHeader title={this.state.studentName+" - "+this.state.programName+" - Preview week "+(this.state.selWeek+1)}>
		                  	</CardHeader>
		        			{previewTable}
		        		</Card>
		        	</Col>
