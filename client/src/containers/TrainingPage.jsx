...
class TrainingPage extends React.Component {
  ...
  calcProgress(weight, reps){
    console.log("[[[[[[calcProgress]", weight, reps)
    let percent=0;
    switch(reps) {
        case 0:
            percent=0
...
//console.log(this.state.selWeek)
  ...
  applyAmrapLinear(){
    ...
    for(let s=0;s<numsets;s++){
      ...
      if(set.amrap==true && s!=0) {
        let searchid = set.id
        let amrap = this.finalWeight((sets[s-1].weight)*this.calcProgress(sets[s-1].weight, parseInt(sets[s-1].reps))+parseInt(set.w1))
  ...
  previousSession(){
    //console.log("previousSession", this.state.selWeek, this.state.currentDay)
    let noweeks =  this.state.program.weeks.length
    //console.log("noweeks",noweeks)
    let week = this.state.selWeek
    //console.log("week",week)
    let nodays = this.state.program.weeks[week].days.length-1
    ...
    if(day>0) {
      this.setState({
        ...
        //console.log("currentDay>now>", this.state.currentDay)
      })
    } else if(day==0) {
      if(week>0) {
        this.setState({
          selWeek: week-1,
          currentDay: this.state.program.weeks[week-1].days.length-1 
        }, function(){
          //console.log("selWeek>now>", this.state.selWeek, this.state.currentDay)
        })
      }  
    }
    ...
    /*if(week>0) {
      this.setState({
        ...
        console.log("selWeek>now>", this.state.selWeek)
      })
    }*/
  ...
  nextSession(){
    //console.log("nextSession", this.state.selWeek, this.state.currentDay)
    let noweeks =  this.state.program.weeks.length-1
    //console.log("noweeks",noweeks)
    let week = this.state.selWeek
    //console.log("week",week)
    let nodays = this.state.program.weeks[week].days.length-1
...
//   console.log(this.state.program.weeks[this.state.selWeek].days[this.state.currentDay].difficulty)
    ...
    <div>
      ...
      <Grid fluid={true}>
        ...
        <Row className="notprint">
              <Col xs={12} md={12} lg={12}>
                  <FlatButton label={this.state.studentName +" , Program: "+this.state.program.name} style={{color:'#FFFFFF'}}/>
                  <FlatButton disabled={this.state.selWeek==0 && this.state.currentDay==0}  onClick={this.previousSession.bind(this)} tooltip="Previous session" style={{marginTop: 10, marginRight:10, backgroundColor:'#42A5F5', color:'#FFFFFF'}} icon={<ChevronLeft />}/>
                  <FlatButton label={" Week "+(parseInt(this.state.selWeek)+1)+", Day "+(parseInt(this.state.currentDay)+1)} style={{color:'#FFFFFF'}}/>
                  <FlatButton onClick={this.nextSession.bind(this)} tooltip="Next sesion" style={{marginTop: 10, marginRight:10, backgroundColor:'#42A5F5', color:'#FFFFFF'}}  icon={<ChevronRight />}/>
              </Col>
        </Row>
        ...
        <Row className="show-grid">
            <Col xs={12} md={12} lg={12}>
              ...
              <Snackbar
                open={this.state.worksnack}
                bodyStyle={{ backgroundColor: '#388E3C' }}
                message="Workout Completed"
                autoHideDuration={3000}
                onRequestClose={this.workSnackClose}
            />
            </Col>
        </Row>
      </Grid>    
